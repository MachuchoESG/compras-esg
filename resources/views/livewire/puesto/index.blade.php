        ...
        <table class="w-full  text-sm text-left rtl:text-right text-gray-500 ">
            ...
            <tbody>
                ...
                <tr class="bg-white border">
                    <td class="px-6 py-3 text-center">{{ $puesto->id }}</td>

                    <td class="px-6 py-3 text-center">{{ $puesto->name }}</td>

                    <td class="px-6 py-3 text-center">{{ $puesto->departamento->name }}</td>

                    <td class="px-6 py-3 text-center">
                        <div class="flex  justify-center">
                            <button wire:click=" edit({{$puesto->id}})" class="flex items-center text-blue-500">
                ...
                @empty
                <tr>
                    <td colspan="2" class="px-6 py-3 text-center">Aun no cuentas con puestos registrados.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
        ...
        <x-slot name='content'>
            <form wire:submit.prevent="update">
                ...
                <div>
                    ...
                    <input class="w-full" wire:model="puestoEdit.name" type="text" id="nombre" name="nombre" required>
                </div>

                <div class="w-full">
                    <x-label for="">Departamento</x-label>
                    <select wire:model="puestoEdit.departamento_id" class="border-1 border-slate-900 rounded-md w-full  text-black text-md capitalize">
