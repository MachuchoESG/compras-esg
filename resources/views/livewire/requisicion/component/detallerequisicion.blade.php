<div class="p-4 border rounded mb-4">
    ...
    <div class="md:flex md:justify-between ">
        ...
        <p>
            @if ($this->estatus())
                <span
                    class="bg-emerald-200 text-emerald-700 text-sm font-bold me-2 px-2.5 py-0.5 rounded">{{ $this->estatus() }}</span>
            @endif
        </p>
                    ...
                    <div x-data="{ openTab: false }" class="border rounded-md mb-4">
                        <div x-on:click="openTab = !openTab" class="border-b">
                            <div class="flex justify-between items-center px-2">
                                ...
                                <span class="text-sm text-gray-500"> Fecha documento
                                    {{ \Carbon\Carbon::parse($cotizacion->created_at)->locale('es')->isoFormat('LL') }}</span>
                            </div>
                                            ...
                                            <tr class="bg-white text-center border-b">
                                                ...
                                                <td class="px-6 py-3">
                                                    @if ($detalle->autorizado)
                                                        <span
                                                            class=" bg-emerald-200  text-emerald-700 text-sm font-bold me-2 px-2.5 py-0.5 rounded">Autorizado</span>
                                                    @else
                                                        @if (in_array($requisicion->estatus_id, [1, 2, 3, 7]))
                                                            <span
                                                                class="bg-emerald-200 text-emerald-700 text-sm font-bold me-2 px-2.5 py-0.5 rounded">Pendiente</span>
                                                        @else
                                                            <span
                                                                class="bg-red-500 text-white text-sm font-bold me-2 px-2.5 py-0.5 rounded">No
                                                                autorizado</span>
                                                        @endif
                                                    @endif
                                                </td>
    ...
    <section class="text-end">
        ...
        @if (
            $requisicion->estatus->name == 'Finalizada' ||
                $requisicion->estatus->name == 'Cotizacion cargada' ||
                $requisicion->estatus->name == 'No autorizada')
            @foreach ($autorizacionHistorial as $historial)
                @if ($historial->autorizado)
                    <p class="text-gray-500 text-md">Autorizado por {{ $historial->autorizador->name }} el
                        {{ \Carbon\Carbon::parse($historial->updated_at)->locale('es')->isoFormat('LLLL') }}</p>
                @else
                    ...
                    @if ($requisicion->estatus->name == 'No autorizada')
                        <p class="text-gray-500 text-md">No autorizado por {{ $historial->autorizador->name }} el
                            {{ \Carbon\Carbon::parse($historial->updated_at)->locale('es')->isoFormat('LLLL') }}</p>
                    @endif
                @endif
            @endforeach
