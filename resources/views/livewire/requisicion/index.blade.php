<div class="p-2 ">
    ...
    <div class="mb-4 w-1/2 flex items-center">
        ...
        <select wire:model.live="paginate" id="paginate" class="w-32 appearance-none bg-white border border-gray-300 hover:border-gray-500 px-4 py-2 rounded shadow leading-tight focus:outline-none focus:shadow-outline-blue focus:border-blue-300">
                ...
                <tr class="relative">
                    <td class="px-2 py-2 sm:px-1 sm:py-1">
                        <div>
                            ...
                            <div class="flex flex-col">
                                <span class="text-sm text-gray-500">Solicitante {{$requisicion->solicitante->name}}</span>
                                <span class="text-sm text-gray-500"> {{ \Carbon\Carbon::parse($requisicion->created_at)->format('d/m/Y') }}</span>
                            </div>
                    ...
                    <div class="flex flex-col">
                        ...
                        @if($this->estatus($requisicion))
                            <span class="bg-emerald-200 text-emerald-700 text-sm font-bold me-2 px-2.5 py-0.5 rounded">{{ $this->estatus($requisicion) }}</span>
                        @endif
                    </p>
                    </div>
                    ...
                    <td class="px-2 py-2 sm:px-1 sm:py-1">{{ \Carbon\Carbon::parse($requisicion->fecharequerida)->format('d/m/Y') }}</td>
                    <td class="px-2 py-2 sm:px-1 ">{{ $requisicion->observaciones }}</td>
                    <td class="px-2 py-2 sm:px-1 sm:py-1 absolute">
